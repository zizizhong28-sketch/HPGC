...
def gen_K_parent_seq(octree, K, nodeNum_list,layer_indexs = []):
    # LevelNum = len(octree) - 1  # reduce one layer when encode, encode M-1 layer of octant of a octree can rebuild M layer octree
    LevelNum = len(nodeNum_list)  # reduce one layer when encode, encode M-1 layer of octant of a octree can 
    nodeNum = sum(nodeNum_list)
    Seq = np.ones((nodeNum + 1, K), "int") * 256
    LevelOctant = np.zeros((nodeNum + 1, K, 2), "int")  # Level ，ind
    Extents = np.zeros((nodeNum + 1, K, 2), "float")  # extent, z_extent
    Pos = np.zeros((nodeNum + 1, K, 3), "float")  # padding 0
    Seq[1, K - 1] = octree[0].node[0].octant
    LevelOctant[0, K - 1, 0] = 1
    LevelOctant[0, K - 1, 1] = 1
    Pos[0, K - 1, :] = octree[0].node[0].center
    Extents[0, K - 1, 0] = octree[0].node[0].extent
    Extents[0, K - 1, 1] = octree[0].node[0].z_extent
    ...
    for L in range(0, LevelNum):
        ...
        for node in octree[L].node:
            if node.is_leaf == True:
                continue
            Seq[n + 1, K - 1] = node.octant
            Seq[n + 1, 0 : K - 1] = Seq[node.parent, 1:K]
            LevelOctant[n + 1, K - 1, :] = [L + 1, node.curind]
            LevelOctant[n + 1, 0 : K - 1] = LevelOctant[node.parent, 1:K, :]
            Extents[n + 1, K - 1, :] = [node.extent,node.z_extent]
            Extents[n + 1, 0 : K - 1] = LevelOctant[node.parent, 1:K, :]
            Pos[n + 1, K - 1] = node.center
    ...
    if len(layer_indexs) >0:
        layer_indexs.sort()
        start = sum(nodeNum_list[:layer_indexs[0]])
        layers_sum = sum(nodeNum_list[layer_indexs[0]:layer_indexs[-1]+1])
        DataStruct = {
        "Seq": DataStruct["Seq"][start:start+layers_sum],
        "Level": DataStruct["Level"][start:start+layers_sum],
        "Extents": DataStruct["Extents"][start:start+layers_sum],
        "Pos": DataStruct["Pos"][start:start+layers_sum],
        }
